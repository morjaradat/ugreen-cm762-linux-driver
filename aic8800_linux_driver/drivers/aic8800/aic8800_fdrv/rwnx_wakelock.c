...
/**
 * rwnx_wakeup_init: MODIFIED for kernel 6.x compatibility.
 * The functions 'wakeup_source_create' and 'wakeup_source_add' are deprecated/removed.
 * We return NULL to bypass this old initialization path entirely, avoiding compilation error.
 */
...
/**
 * rwnx_wakeup_deinit: MODIFIED for kernel 6.x compatibility.
 * The functions 'wakeup_source_remove' and 'wakeup_source_destroy' are deprecated/removed.
 * If a wakeup source was successfully registered via the modern rwnx_wakeup_register,
 * we use the compatible wakeup_source_unregister.
 */
void rwnx_wakeup_deinit(struct wakeup_source *ws)
{
	...
	// Use the modern unregister function if the source was registered.
