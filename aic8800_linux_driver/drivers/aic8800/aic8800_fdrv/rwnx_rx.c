...
/* Compatibility for kernel 6.17+ timer API changes */
 ...
 */
static inline struct rwnx_vif *rwnx_rx_get_vif(struct rwnx_hw *rwnx_hw, int vif_idx)
{
                              ...
                              struct rwnx_sta *sta)
{
    ...
    if (rxvect->format_mod > FORMATMOD_NON_HT_DUP_OFDM)
    {
        ...
        default:
            ...
            nss = rxvect->he.nss;
            sgi = rxvect->he.gi_type;
            rate_idx = N_CCK + N_OFDM + N_HT + N_VHT + N_HE_SU + nss * 216 + mcs * 18 + rxvect->he.ru_size * 3 + sgi;
            break;
        }
    }
    else
                                 ...
                                 u32 vend_it_present)
{
    ...
    // Check if HT
    if ((rxvect->format_mod == FORMATMOD_HT_MF) || (rxvect->format_mod == FORMATMOD_HT_GF))
    {
    ...
    if (rxvect->format_mod == FORMATMOD_VHT)
    {
        ...
        if (rxvect->ch_bw == PHY_CHNL_BW_80)
            *pos++ = 4;
        else if ((rxvect->ch_bw == PHY_CHNL_BW_160) && phy_info->phy_center2_freq)
            *pos++ = 0; // 80P80
...
void arpoffload_proc(struct sk_buff *skb, struct rwnx_vif *rwnx_vif)
{
    ...
    { // IP
        ...
        { // UDP
            ...
            if ((udph->source == __constant_htons(SERVER_PORT)) && (udph->dest == __constant_htons(CLIENT_PORT)))
            { // DHCP offset/ack
...
int reord_process_unit(struct recv_msdu *pframe, struct aicwf_rx_priv *rx_priv, struct sk_buff *skb, u16 seq_num, u8 tid, u8 forward, u8 is_amsdu)
{
    ...
    {
        if (!timer_pending(&preorder_ctrl->reord_timer))
        { // if this timer not countdown, mod_timer timer to start count down
...
int reord_need_check(struct reord_ctrl *preorder_ctrl, u16 seq_num)
{
    ...
    {              // first time seq_num = preorder_ctrl->ind_sn
...
void remove_sec_hdr_mgmt_frame(struct hw_rxhdr *hw_rxhdr, struct sk_buff *skb)
{
    ...
    {
        if (((skb->data[0] & 0x0C) == 0) && (skb->data[1] & 0x40) == 0x40)
        { // protect management frame
            printk("frame type %x\n", skb->data[0]);
...
check_len_update:
    ...
    if (status & RX_STAT_SPURIOUS)
    {
        ...
        {
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 17, 0)
            cfg80211_rx_spurious_frame(rwnx_vif->ndev, hdr->addr2, false, GFP_ATOMIC);
#else
