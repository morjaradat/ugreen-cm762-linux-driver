...
static void rwnx_netdev_setup(struct net_device *dev)
{
    ...
    dev->needed_headroom = max(dev->needed_headroom,
                               (unsigned short)(sizeof(struct rwnx_amsdu_txhdr) + sizeof(struct ethhdr) + 4 + sizeof(rfc1042_header) + 2));
                                     ...
                                     int idx, struct survey_info *info)
{
    ...
    if (rwnx_survey->filled != 0)
    {
        ...
        info->noise = ((IS_2P4GHZ(info->channel->center_freq)) ? DEFAULT_NOISE_FLOOR_2GHZ : (IS_5GHZ(info->channel->center_freq)) ? DEFAULT_NOISE_FLOOR_5GHZ
...
// size_t len = params->len;
    ...
    if (rwnx_hw->roc_elem)
    {
        ...
        if ((rwnx_hw->roc_elem->chan->center_freq != channel->center_freq) || (rwnx_vif->vif_index != rwnx_roc_vif->vif_index))
        {
...
static struct cfg80211_ops rwnx_cfg80211_ops = {
    ...
    //    .set_wiphy_params = rwnx_cfg80211_set_wiphy_params,  // Commented out for kernel 6.17 compatibility
    .set_txq_params = rwnx_cfg80211_set_txq_params,
    //    .set_tx_power = rwnx_cfg80211_set_tx_power,  // Commented out for kernel 6.17 compatibility
